...
- `SKILL_UPDATED`：重新读取新版 `SKILL.md` 和本次需要的参考文件后再继续，不能混用新旧指令。
- `SKILL_UPDATE_MAJOR_AVAILABLE`：跨主版本不自动安装，只提醒管理员。
- `SKILL_UPDATE_WARNING`：网络或校验失败；现有本地版本保持不变，显示一句中文提示后继续任务。

更新器只读取公开 GitHub Release 的稳定版，不读取 `main`。它验证 Skill 身份、稳定 SemVer、SHA-256、压缩路径和必要文件，在同一磁盘的临时目录准备完成后替换；替换失败会恢复旧目录。检测到 `.git` 开发工作树时不自动覆盖，开发者仍使用 Git 更新。
