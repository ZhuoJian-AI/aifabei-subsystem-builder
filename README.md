...
根 skill 同时规定 Alphabet 企业级文件底座：新系统默认使用 ECS 固定数据目录，并从第一天采用可迁移的存储适配层；管理员以后可按清单和校验和迁移到同地域私有 OSS。业务负责人只提出上传、导入或导出需求，不接触服务器路径、阿里云、RAM 或密钥。

管理员还需为每台 ECS 一次性建立 SSH/HTTPS `443` 共用入口并完成外部验收。在普通 TCP `443` 可达的业务网络里，负责人此后只把公网地址、root 账号和密码交给 Codex；不需要 VPN、阿里云账号或已登录的控制台。
